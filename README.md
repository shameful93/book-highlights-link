...
将 EPUB/PDF 电子书转换为 Obsidian 友好的 Markdown，并把 Apple Books 高亮自动链接到段落级 Block ID。

## 中文

### 功能概览

- 一键流程：EPUB/PDF 转换 + 段落 Block ID + 高亮提取 + 自动链接
- 支持提取 EPUB/PDF 图片并转为 Obsidian 图片 wikilink
...
Convert EPUB/PDF books into Obsidian-friendly Markdown, then automatically link Apple Books highlights to paragraph-level Block IDs.

### Features

- One-command pipeline: EPUB/PDF conversion + paragraph Block IDs + highlight extraction + auto linking
- Extract EPUB/PDF images and convert them to Obsidian image wikilinks
...
#### 2) Run all steps in one command (EPUB)
...
#### 2b) Run all steps in one command (PDF)
...
Skip cover/front pages for PDF (first few pages may be scans):
...
- `--epub` / `-e`: EPUB file path (mutually exclusive with `--pdf`)
- `--pdf` / `-p`: PDF file path (mutually exclusive with `--epub`)
- `--asset-id` / `-a`: Apple Books asset ID
- `--output-dir` / `-o`: output directory (default: `./output`)
- `--threshold` / `-t`: match threshold (default: `0.75`)
- `--skip-pages`: skip first N pages of PDF (cover/front matter)
...
- `images/`: extracted EPUB or PDF images
